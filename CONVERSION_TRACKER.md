...
- [x] Footer external links: verified against original `index.html` — the footer renders exactly 4 links (Website, Discord, Patreon, GitHub), matching the C++ implementation. The `::ISSUE_TRACKER` link in `external-links.js` is only used in the crash screen, not the footer.
