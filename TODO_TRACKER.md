...
- **Status**: Pending
- **Details**: The M2 vertex shader declares `uniform mat4 u_bone_matrices[256]` which requires 256 × 4 = 1024 vec4 constant registers for bone data alone. Combined with the other uniforms (`u_view_matrix`, `u_projection_matrix`, `u_model_matrix`, `u_tex_matrix1`, `u_tex_matrix2`, `u_bone_count`, `u_has_tex_matrix1`, `u_has_tex_matrix2`, `u_vertex_shader`, `u_view_up`, `u_time`), the total exceeds NVIDIA's 1024 constant register limit for `#version 300 es` shaders, causing link errors: `error C6020: Constant register limit exceeded`. The original JS app uses WebGL 2.0 where `#version 300 es` is correct and desktop GPU WebGL implementations typically expose higher uniform limits. The C++ port targets desktop OpenGL 4.6 core profile (per project docs), so the shaders should use `#version 460 core` instead of `#version 300 es`. Additionally, bone matrices should be moved to a Shader Storage Buffer Object (SSBO, available in GLSL 430+) or a Uniform Buffer Object (UBO) to avoid hitting uniform register limits entirely. This also affects `src/shaders/wmo.vertex.shader` and `src/shaders/adt.vertex.shader` which also use `#version 300 es`. All shaders need to be updated to desktop GLSL versions and remove GLES-specific qualifiers (`precision highp float`). The renderer code in `M2RendererGL.cpp` (lines 1482–1488) that uploads bone matrices via `glUniformMatrix4fv` would need to be updated accordingly to use SSBO/UBO buffer uploads.
...
- **Status**: Pending
- **Details**: The hamburger menu (≡ icon) dropdown in the header does not stay open when clicked. Multiple issues contribute to this:
  1. **Z-ordering**: The popup window `##MenuExtra` (app.cpp lines 613–636) is created with `ImGuiWindowFlags_NoFocusOnAppearing`, which prevents it from coming to the front of the window stack. On the first open, the newly created window IS at the top, but when re-opened (after being hidden), it retains its old position in ImGui's focus order and renders BEHIND the `##AppContent` window (app.cpp line 774) which covers the entire content area below the header. The user never sees the menu.
  2. **Not using `context_menu::render()`**: The JS version uses the `<context-menu>` Vue component (`src/js/components/context-menu.js`), which the C++ port already has as `context_menu::render()` in `src/js/components/context-menu.cpp`. However, the hamburger menu in `app.cpp` does NOT use this component — it implements its own inline popup with `ImGui::Begin/End`, bypassing all the context menu behavior (positioning, close-on-mouseleave, buffer zone).
  3. **Toggle vs set**: The JS click handler uses `contextMenus.stateNavExtra = true` (always opens, index.html line 18). The C++ uses `!stateNavExtra` (toggle, app.cpp line 597). Combined with the close check at line 639, this could cause the menu to close on the same frame it opens under certain conditions.
  4. **Missing 20px buffer zone**: The JS context-menu has a `.context-menu-zone` CSS class (app.css lines 892–899) that extends the hover area 20px in all directions, preventing premature `@mouseleave` closure. The C++ implementation has no equivalent.
  5. **WindowBg style applied after Begin**: `PushStyleColor(ImGuiCol_WindowBg, COLOR_BG_DARK)` at app.cpp line 618 is called INSIDE `Begin()`, so the window background has already been rendered with the default color. This should be applied with `SetNextWindowBgAlpha` or `PushStyleColor` BEFORE `Begin()`.
  - **Fix approach**: Refactor the hamburger menu to either use `context_menu::render()` (matching the JS architecture), or use ImGui's built-in popup API (`ImGui::OpenPopup`/`ImGui::BeginPopup`) which handles z-ordering and close-on-click-outside automatically. If keeping the custom implementation, remove `NoFocusOnAppearing` from `##MenuExtra` (or call `ImGui::SetWindowFocus()` when opening) so the popup renders on top of the content area.
