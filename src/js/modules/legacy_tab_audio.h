...
/**
 * Legacy audio tab module (ImGui immediate-mode equivalent).
 *
 * JS equivalent: Vue component with register(), template, methods
 * (handle_listbox_context, copy_file_paths, copy_export_paths,
 * open_export_directory, toggle_playback, handle_seek, export_selected),
 * and mounted().
 *
 * Provides: Audio nav button (MPQ only), sound file listbox with
 ...
 * JS equivalent: register() { this.registerNavButton('Audio', 'music.svg', InstallType.MPQ) }
 */
void registerTab();

/**
 * Initialize the legacy audio tab (player init, sound list, selection watch).
 * JS equivalent: mounted()
 */
void mounted();

/**
 * Render the legacy audio tab widget using ImGui.
