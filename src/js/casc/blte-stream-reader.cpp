...
BLTEStreamReader::BLTEStreamReader(const std::string& hash, const BLTEMetadata& metadata,
	std::function<BufferWrapper(size_t)> blockFetcher, bool partialDecrypt)
...
BufferWrapper BLTEStreamReader::getBlock(size_t blockIndex) {
	...
	BufferWrapper rawBlock = blockFetcher(blockIndex);
	...
	BufferWrapper decoded = _decodeBlock(std::move(rawBlock), blockIndex);
	...
	switch (flag) {
		case 0x45: { // Encrypted.
			try {
				...
				return _decodeBlock(std::move(decrypted), index);
