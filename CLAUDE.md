...
- **Do not "fix" perceived JS bugs.** If the JS source does something that looks like a bug (off-by-one, wrong variable, unreachable branch, etc.), port it exactly as-is. The C++ version must reproduce the same behavior, not "improve" it. Do not silently correct JS logic during conversion. If you believe something is genuinely a bug, mention it to the user — do not fix it unilaterally.
