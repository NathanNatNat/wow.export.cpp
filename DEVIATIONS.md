...
## Bug Fixes (JS bugs corrected in C++)

### [GLTFWriter.cpp] Animation channel node target index
- **JS Source**: `src/js/3D/writers/GLTFWriter.js` lines 621, 761, 891
- **Reason**: JS animation channels always use `nodeIndex + 1` as the target node. When `modelsExportWithBonePrefix` is false, `nodeIndex + 1` points one past the bone node — an off-by-one bug. C++ uses the correct node index in both prefix and non-prefix modes.
- **Impact**: Exported glTF animations correctly target bone nodes regardless of prefix mode setting.

### [casc-source.cpp] parseEncodingFile page boundary calculation
- **JS Source**: `src/js/casc/casc-source.js` line 470
- **Reason**: JS uses `pageStart + pagesStart` as the inner loop bound when iterating encoding pages. `pagesStart` is the byte offset where all pages begin, not the page size. The correct bound is `pageStart + cKeyPageSize`. C++ correctly uses `pageStart + cKeyPageSize`.
- **Impact**: C++ correctly parses CASC encoding file pages within their actual boundaries.
